...
void compress_stream(void)
{
    ...
    int compressed_size = LZ4_compress_fast_continue(stream, src_data, dest_data, src_len, max_compressed_size, acceleration);
...
void decompress_stream(void)
{
    ...
    if (!original_size)
    {
        HANDLE_ERROR("read max must be grater than zero");
    }
    ...
    int decompressed_size = LZ4_decompress_safe_continue(stream_decode, src_data, dest_data, src_len, original_size);
